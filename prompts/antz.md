...
Route on what is on disk, never on memory of an earlier session. If what is there belongs to a different feature than the request, say so and stop — starting fresh means deleting `.antz/`, never resuming someone else's change. Otherwise the most advanced artifact present tells you where to enter:
...
1. Run antz-scout (single) with the request as its task, then step 2.
...
3. Run antz-planner (single) to write `.antz/02-plan.md`, then step 4.
