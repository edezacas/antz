# Domain: bench-runner
# Change: telemetry-benchmark (sub-spec 5 of 6)
#
# Goal: the runner CLI -- "bench/antz-bench.sh" -- that ties the fixture, the
# adapters, the schema, and the report together: client detection and forcing,
# N sequential repetitions per client, the per-repetition deadline, the JSONL
# and run-dir outputs, and the exit contract. One invocation answers "how does
# antz fare on this machine, on this fixture, for these clients".
...
## Out of scope
- Parallel repetitions or multi-machine distribution (sequential keeps runs
  comparable).
- Resuming an interrupted run from a partial JSONL.
- Any flag beyond the pinned surface (a future change extends the CLI).
