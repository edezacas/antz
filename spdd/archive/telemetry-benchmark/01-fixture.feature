...
# Change: telemetry-benchmark (sub-spec 1 of 6)
#
# Goal: a self-contained benchmark fixture checked into antz: plain files plus
# a scenario descriptor under "bench/fixture/", from which the harness
# materializes a fresh git repo per repetition. The fixture drives one full
# specifier -> coder -> verifier flow on a small, deterministic request. The
# checked-in tree carries no git data of its own (no nested ".git"); git
# exists only in the per-repetition materialized copy, under temp space.
...
## Out of scope
- Multiple scenarios per fixture (one checked-in scenario now; the schema
  carries a scenario field for future growth).
- Any tuning of the request aimed at making outcomes deterministic -- real
  client runs are measurements, not fixtures; only the request and starting
